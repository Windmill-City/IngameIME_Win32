...
class ComQIPtr : public ComPtr<T>
{
  ...
  public:
    constexpr ComQIPtr(const IID iid) noexcept
        : iid(iid)
    {
    }

    constexpr ComQIPtr(const IID iid, std::nullptr_t) noexcept
        : iid(iid)
    {
    }

    template <class Q>
    ComQIPtr(const IID iid, const ComPtr<Q>& other) noexcept
        : iid(iid)
    {
        IUnknown* p;
        if (!other || FAILED(other->QueryInterface(iid, (void**)&p)))
        {
            this->ptr = nullptr;
        }
        else
