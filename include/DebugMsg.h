...
const std::vector<UINT> IgnoredMsg = {
    ...
    WM_GETICON,        WM_NCACTIVATE, WM_NCLBUTTONDOWN, WM_SYSCOMMAND,    WM_LBUTTONDOWN,       WM_LBUTTONUP,
    WM_CAPTURECHANGED, WM_MOVING,     WM_ENTERSIZEMOVE, WM_EXITSIZEMOVE,  WM_NCMOUSELEAVE,      WM_NCHITTEST,
    WM_SETCURSOR,      WM_MOUSEFIRST, WM_NCMOUSEMOVE,   WM_MOUSELEAVE,    WM_KEYDOWN,           WM_KEYUP};

std::map<UINT, size_t> UsedMsg = {};

// These from https://wiki.winehq.org/List_Of_Windows_Messages
const std::map<UINT, std::wstring> MsgMap = {{0, L"WM_NULL"},
                                             {1, L"WM_CREATE"},
                                             {2, L"WM_DESTROY"},
                                             {3, L"WM_MOVE"},
                                             {5, L"WM_SIZE"},
...
{
    for (auto&& kv : UsedMsg) { wprintf(L"[%04x](%s): %zu\n", kv.first, GetMessageText(kv.first).c_str(), kv.second); }
}

void ShowMessageText(UINT msg) noexcept
{
    // Dont show Ignored msgs
    if (std::find(IgnoredMsg.begin(), IgnoredMsg.end(), msg) == IgnoredMsg.end()) {
        auto text = GetMessageText(msg);
        if (!text.empty()) wprintf(L"WndProc: [%04x](%s)\n", msg, text.c_str());
    }
}