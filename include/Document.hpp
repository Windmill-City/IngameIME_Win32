...
namespace libtf
{
	class TF_API Document : private COMBase,
	...
	public:
		...
		HRESULT __stdcall OnStartComposition(ITfCompositionView *pComposition, BOOL *pfOk) override
		{
			*pfOk = TRUE;
			if (m_sigComposition)
				m_sigComposition(new CompositionEventArgs(CompositionState::StartComposition));
			return S_OK;
		}
		...
		{
			if (m_sigGetTextExt)
				m_sigGetTextExt(acpStart, acpEnd, prc, pfClipped);
...
#define TEXT_BUF_SIZE 65
		HRESULT __stdcall DoEditSession(TfEditCookie ec) override
		{
			...
			if (m_fClean)
			{ //We need to clear the commited text
		...
		HRESULT __stdcall OnEndEdit(ITfContext *pic, TfEditCookie ec, ITfEditRecord *pEditRecord) override
		{
			if (m_sigComposition)
			{
				...
				if (cch > 0)
				{ //after committed, the context has set to empty
