...
namespace libtf {
	...
	HRESULT __stdcall TextStore::OnUpdateComposition(ITfCompositionView* pComposition, ITfRange* pRangeNew)
	{
		//CompositionUpdate at _UnlockDocument()
		return S_OK;
	}
	...
	void TextStore::_UnlockDocument()
	{
		...
		if (m_fhasEdited) {
			...
			if (m_Commit) {
				...
				//Notify App we have text committed
				m_sigComposition(this, new CompositionEventArgs(m_StoredStr.substr(m_CommitStart, commitLen)));
				//Remove the Committed text and notify IME
